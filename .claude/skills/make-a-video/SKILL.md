...
**First, check the Second Brain for existing material on this topic** (find the Second Brain vault among your working directories by its signature file `knowledge/_index.md`): look in `projects/video-production/scripts/` for an already-drafted script, and `knowledge/` + `projects/video-production/` for research and angles. If something relevant exists, offer it as the starting point instead of asking the user to supply from scratch. When drafting, write in your brand voice per `knowledge/concepts/brand-voice.md` + `script-structure.md`.

1. Script source? (use existing brain script · paste · outline → I'll draft in your brand voice · I'll record · TTS from text · no narration)
...
**Before asking the user anything, read your brand sources (if configured), then inventory assets.** (See `DESIGN.md` at the workspace root for the source map.)

1. Read `DESIGN.md` — it maps the source-of-truth repos and how to locate them.
2. From your **design-system source**, if you've configured one (locate it among your working directories per `DESIGN.md`): read its color/type tokens and note the logo. These ARE the answers to Q11–Q14 below — pre-fill them, don't ask. If you have no design system yet, skip this and ask the user (or fall back to the project's `brand-tokens.css` placeholder / MOTION_PHILOSOPHY defaults).
3. From **Daniel's Second Brain** vault (find it by its signature file `knowledge/_index.md`): read `knowledge/concepts/brand-voice.md` and `script-structure.md` for voice/tone and pacing feel.
4. Then inventory `<workspace-root>/assets/` and any project `assets/` folder.

**Only ask the user about what these sources don't answer** — e.g. this video's one accent emphasis, reference videos, music choice. Never re-ask anything the sources already cover. If either source isn't accessible (not in your working directories), tell the user to set up `.claude/settings.local.json` (see `.claude/settings.local.json.example`) and fall back to asking the user / MOTION_PHILOSOPHY defaults.

Then ask progressively for the gaps — they don't need answers to all of these:
...
7. MOTION_PHILOSOPHY discipline + your brand (your palette · your headline type treatment · your textures · transitions) or a different feel?
...
4. Copy supplied assets into `<project-folder>/assets/`. **Also copy the brand binaries the renderer needs (your fonts and logo) from your design-system source, if configured, into `<project-folder>/assets/`.** Per `DESIGN.md`, specs are referenced live but the binary fonts/logo must be physically present for deterministic rendering (no render-time network fetches — Render Contract rule 11).
5. Create `<project-folder>/assets/style-profile.md` from Gate 3 + your `DESIGN.md` sources — single source of truth for palette/fonts/logo, with values from your design system (or the project's `brand-tokens.css` placeholder) and the `logo`/`font` fields pointing at the local copies you made in step 4.
