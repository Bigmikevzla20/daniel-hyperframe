...
## Before asking anything — read sources, then inventory

**Read the brand + content sources first** (see `DESIGN.md` at the workspace root for how to locate them), then inventory local assets. Pre-fill every answer the sources already provide; only ask the user about the gaps.

1. **your design system** repo (find by signature file `colors_and_type.css`): `colors_and_type.css`, `design.md`, `fonts/`, `assets/` → answers palette, type, logo (Q11–Q14).
2. **Daniel's Second Brain** vault (find by signature file `knowledge/_index.md`): `knowledge/concepts/brand-voice.md` + `script-structure.md` → voice/tone; `projects/video-production/scripts/` + `knowledge/` → script + research (Q7).
3. **Local assets** — what already exists so you don't ask for supplied files:
...
- MOTION_PHILOSOPHY discipline + your brand: dark canvas · your headline treatment (define it in your brand tokens / design system) · perspective grid · whip-pan transitions · 4–6s outro hold
