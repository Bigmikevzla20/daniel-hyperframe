...
| Headline text gradient | Your headline treatment — define it in your brand tokens / design system (e.g. a `background` gradient with `-webkit-background-clip: text; color: transparent;`). Pull the exact values from `DESIGN.md` rather than hardcoding them here. |
