...
- Inter 900 (Black), 46–58px (for 1080 width), 100% white base
- Active word: scale-1.08 pop + color change to accent (use your brand accent — read it from your `DESIGN.md` sources or the project's `brand-tokens.css`)
...
For scenes 3–5 of a 15–20s short (the "middle grind" where attention drops the hardest), lean on visuals that read as information — bar races, stat grids with counting numbers, heatmaps, sparklines, flowcharts, dashboard chrome with telemetry ticking, pain-point grids that flash red in sequence. may-shorts-18 v1 scene 4 was a radar-rings + terminal-chip + sparks combo — functional but decorative, and the reviewer called it "bland." v2 replaced it with a 3×3 pain-point grid lighting up red-orange in sequence + a sparkline stroke-drawing with a YOU-ARE-HERE marker + the payoff slam — same time budget, much higher engagement.
...
The original `may-shorts-18/19` example projects have been removed from this repo. The face-mode choreography, karaoke-caption (`shift()`), and the four lessons above are captured in this skill — apply them directly. For an example to model structure from, read any existing project under `video-projects/` (e.g. `sample-project/`).
...
- `DESIGN.md` (workspace root) — your brand source map: read your design system / `brand-tokens.css` for hex codes, fonts, and logo
